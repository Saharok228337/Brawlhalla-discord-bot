...
async def add(interaction: discord.Interaction, brawlhalla_id: int):
    ...
    if response.status_code == 200:
        ...
        embed.set_thumbnail(
            url='https://static.wikia.nocookie.net/brawlhalla_gamepedia/images/1/14/Brawlhalla_Logo_100M_Full.png/revision/latest/scale-to-width-down/340?cb=20230520001004'
        )
        embed.add_field(
            name='ІГРИ',
            value=f'Зіграно ігор: {data["games"]}\nПеремог: {data["wins"]}\nПоразок: {data["games"] - data["wins"]}',
            inline=False,
        )
        embed.add_field(
            name=f'Улюблений персонаж: {dataL["legends"][0]["legend_name"]}',
            value=f'Ігор зіграно: {favourite["games"]}\nПеремог: {favourite["wins"]}\nПоразок: {favourite["games"] - favourite["wins"]}\n{favourite["ko_weapon_one"]} вбивств за {dataL["legends"][0]["weapon_one"]}\n{favourite["ko_weapon_two"]} вбивств за {dataL["legends"][0]["weapon_two"]}',
            inline=False,
        )
    if response1v1.status_code == 200:
        embed.add_field(
            name='RANKED 1v1',
            value=f'{data1v1["games"]} ігор зіграно ({data1v1["wins"]} перемог | {data1v1["games"] - data1v1["wins"]} поразок)\nРанг: {data1v1["tier"]}\nElo: {data1v1["rating"]}\nPeak Elo: {data1v1["peak_rating"]}\nНайкращий персонаж в цьому сезоні: **{dataL1["legends"][0]["legend_name"]}** (elo: {best["rating"]})\n{data1v1["global_rank"]}th місце в світі\n{data1v1["region_ranks"][0]["rank"]}th місце в {data1v1["region_ranks"][0]["region"]}',
            inline=False,
        )
    if response2v2.status_code == 200:
        embed.add_field(
            name='RANKED 2v2',
            value=f'**{Bteam["username_one"]} і {Bteam["username_two"]}**\n*Найкраща команда за ELO*\n{Bteam["games"]} ігор зіргано ({Bteam["wins"]} перемог | {Bteam["games"] - Bteam["wins"]} поразок)\nРанг: {Bteam["tier"]}\nElo: {Bteam["rating"]}\nPeak Elo: {Bteam["peak_rating"]}\n{Bteam["global_rank"]}th місце в світі\n{Bteam["region_ranks"][0]["rank"]}th місце в {Bteam["region_ranks"][0]["region"]}',
            inline=False,
        )
        if Bteam != Bteam2:
            embed.add_field(
                value=f'**{Bteam2["username_one"]} і {Bteam2["username_two"]}**\n*Найкраща команда за кількістю ігор*\n{Bteam2["games"]} ігор зіргано ({Bteam2["wins"]} перемог | {Bteam2["games"] - Bteam2["wins"]} поразок)\nРанг: {Bteam2["tier"]}\nElo: {Bteam2["rating"]}\nPeak Elo: {Bteam2["peak_rating"]}\n{Bteam2["global_rank"]}th місце в світі\n{Bteam2["region_ranks"][0]["rank"]}th місце в {Bteam2["region_ranks"][0]["region"]}',
                inline=False,
            )
    if response3v3.status_code == 200:
        ...
        embed.add_field(
            name='RANKED 3v3',
            value=f'{data3v3["games"]} ігор зіграно ({data3v3["wins"]} перемог | {data3v3["games"] - data3v3["wins"]} поразок)\nРанг: {data3v3["tier"]}\nElo: {data3v3["rating"]}\nPeak Elo: {data3v3["peak_rating"]}\n{data3v3["global_rank"]}th місце в світі\n{data3v3["region_ranks"][0]["rank"]}th місце в {data3v3["region_ranks"][0]["region"]}',
            inline=False,
        )
    ...
    if responseG.status_code != 200:
        embed.set_footer(text='Поки без клану')
    if response.status_code == 200:
...
async def add(interaction: discord.Interaction, guild_id: int):
    ...
    if responseGl.status_code == 200:
        ...
        embed = discord.Embed(
            title=f'{dataGl["name"]}', description=f'"{dataGl["notice"]}"', color=discord.Color.green()
        )
        embed.set_thumbnail(
            url='https://static.wikia.nocookie.net/brawlhalla_gamepedia/images/1/14/Brawlhalla_Logo_100M_Full.png/revision/latest/scale-to-width-down/340?cb=20230520001004'
        )
        ...
        embed.add_field(
            name='ІНФОРМАЦІЯ', value=f'Guild Points: {dataGl["guild_points"]}\nXP: {dataGl["xp"]}', inline=False
        )
        if 'rank' in dataGl:
